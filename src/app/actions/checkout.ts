...
export async function checkout(productId: string, purchaseType: 'single' | 'multiple', userId: string) {
  ...
  try {
    ...
    const { data: purchase } = await supabase
      ...
      .insert({
        ...
        amount_paid: purchaseType === 'single' ? product.single_purchase_price ?? 0 : product.multiple_purchase_price ?? 0
      })
      .select()
      .single()
    ...
    redirect(`/i/${productId}?error=checkout_failed`)
  }
}
